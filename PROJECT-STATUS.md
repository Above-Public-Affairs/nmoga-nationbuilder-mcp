...
- [x] **Five code-quality fixes (2026-08-12):** same-day review found and fixed: `add_tags_to_person` could create near-duplicate tags on a casing mismatch (now resolves case-insensitively like its sibling tools); concurrent 401s could race two OAuth refreshes against NationBuilder's refresh-token rotation (now single-flighted in `src/oauth.ts`); outbound NationBuilder requests had no timeout (now 15s per attempt); the rate limiter was built fresh per HTTP session instead of once per process, so concurrent connector sessions each believed they had the full 200-req/10s budget (now one shared client/limiter, plus 429 responses honor `Retry-After`); `.env.example`'s dead `RAILWAY_API_TOKEN` block removed. See CHANGELOG.md.
